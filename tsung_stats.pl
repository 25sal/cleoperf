    ...
    while (<FILE>) {
        if (/^stats: (\S+)\s+(.*)$/) {
            ...
            if ($type =~ /^cpu:/ ) {
             ...
             # print STDOUT $type.",".$mean.",".$max.",".$min.",".$meanfb."\n";
...
sub csv_report {
    ...
    open(CSVF,">./metrics.csv") or die "can't open metrics.csv: $!";
    select CSVF;
    ...
    foreach my $type ("mean"){#, "maxmean", "minmean") {
        ...
        foreach my $data (keys % {$maxval->{$type}} ) {
            ...
            if($maxval->{$type}->{$data}){
              print   $data.",".$type.",".$maxval->{$type}->{$data}."\n";}


}
    foreach my $data (keys % {$maxval->{'rate_count'}} ) {
        next if ($data =~ m/^size/);
       print    $data.",ratemean,".$maxval->{'rate_mean'}->{$data}."\n";
    }
    
    foreach my $size ("size_rcv", "size_sent") {
	 
        if ($maxval->{rate}->{$size}) {
              print   $size.",rate,".$maxval->{rate}->{$size}."\n".$size.",maxmean,".$maxval->{maxmean}->{$size}."\n" ;
