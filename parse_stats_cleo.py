...
parameters = ['tr_start_conversation', 'tr_mongo', 'tr_change_canvas', 'tr_zoom', 'session', 'page', 'request', 'tr_init_load_page', 'connect', 'tr_change_manually_canvas_2', 'tr_change_manually_canvas']



for experiment in [base_cleo]:
    df = pd.DataFrame(
        columns=['users', 'time', 'parameter', 'rate', 'mean', 'stddev', 'max', 'min', 'meanfb', 'countfb'])
    df_users = pd.DataFrame(columns=['users', 'time', 'connected'])
    for test in test_set:
        start_time = None
        stat_file = open(base_dir+"/"+experiment+str(test)+"/tsung.log", "r")
        print(experiment+str(test))
    ...
    df.to_csv(experiment+'df_stats.csv')
    df_users.to_csv(experiment+'df_users.csv')
